...
class SelectStationView: UIView , MCTableViewIndexDelegate{
    ...
    func Delegate(delegate:SelectStationViewDelegate){
      _delegate = delegate
        ...
        var hh =  _tableViewIndex.indexes.count * 14
        rect = CGRectMake(rect.origin.y, rect.origin.y, rect.width, CGFloat(_tableViewIndex.indexes.count * 14))
    ...
    func reloadData(){
       _tableView.reloadData()
        ...
        rect = CGRectMake(rect.origin.x, y, rect.width, height)
        _tableViewIndex.frame = rect
        _tableViewIndex.delegate(self)
